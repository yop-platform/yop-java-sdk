...
//        params.put(paramKey, paramPlainValue);
        ...
        // 签名头
        headers.put(AUTHORIZATION, signFormRequest(headers, params));

        // 构造请求
        HttpUriRequest request = buildFormHttpRequest(serverRoot + apiUri,
                HttpMethodName.POST, headers, params);
    ...
    private static String signFormRequest(Map<String, String> headers, Multimap<String, String> params) throws Exception {
        // A.构造认证字符串
        String authString = buildAuthString();

        // B.获取规范请求串
        SortedMap<String, String> headersToSign = getHeadersToSign(headers, DEFAULT_HEADERS_TO_SIGN);
        String canonicalRequest = buildCanonicalRequest(authString, headersToSign);
    ...
    private static String buildCanonicalRequest(String authString,
                                         SortedMap<String, String> headersToSign) {
    ...
    private static ByteArrayInputStream getContentStream(Multimap<String, String> params) throws Exception {
        String encodedParameters = getCanonicalQueryString(params.asMap(), true);
        byte[] bytes;
        if (StringUtils.isEmpty(encodedParameters)) {
            bytes = new byte[0];
        } else {
            bytes = encodedParameters.getBytes(DEFAULT_ENCODING);
        }
    ...
    protected static HttpUriRequest buildFormHttpRequest(String requestUrl, HttpMethodName requestMethod,
